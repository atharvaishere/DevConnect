...
function Project() {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      setLoading(true);
      try {
        console.log('Fetching project with ID:', id, 'Token:', user?.token);
        const headers = user?.token ? { Authorization: `Bearer ${user.token}` } : {};
        const projectRes = await axios.get(`${process.env.REACT_APP_API_URL}/api/projects/${id}`, { headers });
        console.log('Project response:', projectRes.data);
        setProject(projectRes.data);

        const commentsRes = await axios.get(`${process.env.REACT_APP_API_URL}/api/comments/project/${id}`, { headers });
                    ...
                    {editCommentId === comment._id ? (
                      ...
                      <>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                          By {comment.userId?.username || 'Unknown'} on {new Date(comment.createdAt).toLocaleDateString()}
                        </p>
